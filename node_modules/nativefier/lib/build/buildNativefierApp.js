...
async function copyIconsIfNecessary(options, appPath) {
    ...
    if (options.packager.platform === 'darwin' ||
        ...
        if (options.nativefier.tray) {
            ...
            await helpers_1.copyFileOrDir(`${path.dirname(options.packager.icon)}/${trayIconFileName}`, destIconPath);
        }
        else {
...
function isUpgrade(rawOptions) {
    return (rawOptions.upgrade !== undefined &&
...
async function buildNativefierApp(rawOptions) {
    ...
    if (isUpgrade(rawOptions)) {
        ...
        const oldApp = upgrade_1.findUpgradeApp(rawOptions.upgrade.toString());
        if (oldApp === null) {
            throw new Error(`Could not find an old Nativfier app in "${rawOptions.upgrade}"`);
        }
        rawOptions = upgrade_1.useOldAppOptions(rawOptions, oldApp);
