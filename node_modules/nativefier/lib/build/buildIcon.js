...
function convertIconIfNecessary(options) {
    ...
    if (options.packager.platform === 'win32') {
        ...
        try {
            const iconPath = iconShellHelpers_1.convertToIco(options.packager.icon);
    ...
    if (options.packager.platform === 'linux') {
        ...
        try {
            const iconPath = iconShellHelpers_1.convertToPng(options.packager.icon);
            options.packager.icon = iconPath;
            return;
        }
        catch (error) {
    ...
    try {
        if (!iconIsIcns(options.packager.icon)) {
            const iconPath = iconShellHelpers_1.convertToIcns(options.packager.icon);
            options.packager.icon = iconPath;
        }
        if (options.nativefier.tray) {
            iconShellHelpers_1.convertToTrayIcon(options.packager.icon);
        }
    }
    catch (error) {
